...
Sleep(5);
add_log(". Leg Engine CVAR file", p_lfc.cvarFullscreen);
add_log("gl_Fullscreen %i", p_lfc.cvarFullscreen);
add_log(" ");
add_log("gl_AF %i", p_lfc.cvarAF);
add_log(" ");
...
add_log(" ");
add_log("glsl_Shaders %i", p_lfc.cvarShaders);
add_log(" ");
...
void cCVARS::Init(void)
{
	...
	ADDINT(controls_MousePitchSpeed, 1)
	ADDINT(controls_MouseYawSpeed, 1)
